...
const config: Record<Confidence, { label: string; icon: typeof Check; classes: string }> = {
  ...
  low: { label: 'Unsure', icon: HelpCircle, classes: 'border border-dashed border-ink-faint text-ink-soft' },
}

export function ConfidenceTag({ level, className }: { level: Confidence; className?: string }) {
  const c = config[level]
  return (
    <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-label', c.classes, className)}>
