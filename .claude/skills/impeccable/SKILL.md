...
1. **Focus PT install (instructions only):** the Impeccable launcher, binary, hooks, and live-browser tools are deliberately not installed in this repo. Do not run, download, or install `scripts/impeccable` or `npx impeccable`. Instead read the project context directly: `CLAUDE.md` (product context, standing in for PRODUCT.md) and `DESIGN.md` (the committed visual world). Both are binding. Where anything in this skill conflicts with DESIGN.md or CLAUDE.md, those files win. Every surface in this app is **Operate** mode.
...
- **Otherwise:** treat the request as general design work. In this repo CLAUDE.md is the product context: do not create PRODUCT.md. A new surface goes through new-work within the DESIGN.md world; a narrow refinement of existing code proceeds on the incumbent implementation as CLAUDE.md and DESIGN.md direct.
- `teach` aliases `init`. `craft` is a deprecated alias for ordinary new-work and adds nothing. `shape` owns task discovery, then enters new-work only for visual-world and surface-concept decisions.

After init writes PRODUCT.md, resume; init loads the native platform reference itself when the platform it recorded is `ios`, `android`, or `adaptive`.
